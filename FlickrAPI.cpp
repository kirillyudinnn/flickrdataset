...
std::string FlickrAPI::getRequest(int page_number) {
    return API_MAIN + this->API_KEY + req + tag + tag_mode + license + 
sort + content + per_page + page + std::to_string(page_number) + format + callback;
};

std::string FlickrAPI::getPhotoURL(std::vector<std::string>& fields) const {
...
std::string FlickrAPI::getJsonRequest(int page_number) {
    CURL* curl;

    std::string str_response;
    curl = curl_easy_init();
    curl_easy_setopt(curl, CURLOPT_URL, getRequest(page_number).c_str());
...
void FlickrAPI::processPhoto(int batch_index, int count, int max_json_length) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        fs::path file_path = this->save_path / (std::to_string(i + max_json_length * batch_index) + ".jpg");
...
void FlickrAPI::uploadPhoto(std::string text, std::string tags, int photo_count) {
